...
<script type="text/javascript">
    //<![CDATA[
    $(function() {
        $("div#pageContent").load("{MODULE_URL}=cat&list&parentid={parentid}&random=" + nv_randomPassword(10));
    });
    ...
    <form id="addCat" method="post" action="{ACTION_URL}">
        ...
        <div class="table-responsive">
            <table class="table table-striped table-bordered table-hover">
                ...
                <tbody>
                <tr>
                    <td>{LANG.sl_parentid}</td>
                    <td>
                        <select class="form-control" name="parentid">
                            <option value="0"> ---------- </option>
                            <!-- BEGIN: cat -->
                            <option value="{CAT.id}"{CAT.sl}>{CAT.title_show}</option>
                            <!-- END: cat -->
                        </select>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <select class="form-control" name="cattype">
                            <!-- BEGIN: cattype -->
                            <option value="{CATTYPE.key}"{CATTYPE.sl}>{CATTYPE.title}</option>
                            <!-- END: cattype -->
                        </select>
                    </td>
                </tr>
...
<div class="table-responsive">
    <table class="table table-striped table-bordered table-hover">
        ...
        <tr>
            ...
            <td> <a href="{LOOP.link_cat}">{LOOP.title_show}</a> <span class="red">({LOOP.numsubcat})</span></td>
