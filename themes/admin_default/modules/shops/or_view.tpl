...
<div class="text-center">
	<form class="form-inline" action="" method="post" name="fpost" id="post"><input type="hidden" value="{order_id}" name="order_id"><input type="hidden" value="1" name="save">
		...
		<!-- END: onpay -->
		<!-- BEGIN: unpay -->
		<input class="btn btn-success" type="button" value="{LANG.order_submit_shiping}" id="click_shiping">
		<input class="btn btn-danger" type="button" value="{LANG.order_submit_unpay}" id="click_pay">
		<!-- END: unpay -->
		<!-- BEGIN: shiped -->
		<input class="btn btn-success" type="button" value="{LANG.order_submit_shiped}" id="click_shiped">
		<!-- END: shiped -->
...
<script type="text/javascript">
	$(function() {
        ...
        $('#click_shiping').click(function(event) {
            event.preventDefault();
            if (confirm("{LANG.order_submit_shiping_comfix}")) {
                $.ajax({
                    type : "POST",
                    url : '{URL_ACTIVE_SHIPING}',
                    data : 'save=1',
                    success : function(data) {
                        alert(data);
                        window.location = "{URL_BACK}";
        ...
        $('#click_shiped').click(function(event) {
            event.preventDefault();
            if (confirm("{LANG.order_submit_shiped_comfix}")) {
                $.ajax({
                    type : "POST",
                    url : '{URL_ACTIVE_SHIPED}',
                    data : 'save=1',
                    success : function(data) {
                        alert(data);
                        window.location = "{URL_BACK}";
