...
<div class="sendding">
	...
	<div class="row">
		<div class="col-xs-24 col-sm-8">
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<!-- BEGIN: btn_control -->
					<p class="text-center" id="btn-control"><em title="{LANG.start}" class="fa fa-play-circle fa-5x fa-pointer" onclick="nv_sendmail({ROWSID}, {NEXTCUSTOMERID}); return false;">&nbsp;</em></p>
					<!-- END: btn_control -->
					<div class="progress">
						<div class="progress-bar" id="progress-bar" role="progressbar" aria-valuemin="0" aria-valuemax="100" style="width: {PERCENT}%"><span id="totalsend">{TOTALSENDER}</span>/{TOTAL} <span class="text-lowercase">{LANG.sendstatus_5}</span></div>
					</div>
					<div class="row send-count">
						<div class="col-xs-24 col-sm-12 text-center">
							<label class="text-uppercase"><strong>{LANG.sendstatus_1}</strong></label>
							<span class="show" id="countsuccess">{COUNTSUCCESS}</span>
						</div>
						<div class="col-xs-24 col-sm-12 text-center">
							<label class="text-uppercase"><strong>{LANG.sendstatus_3}</strong></label>
							<span class="show" id="counterror">{COUNTERROR}</span>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="col-xs-24 col-sm-16">
			<div class="panel panel-default">
				<div class="panel-heading" id="title-emaillist">{LANG.phonelist}</div>
						...
						<tbody>
							...
							<tr id="row_{DATA.id}" data-customerid="{DATA.id}">
								...
								<td id="sendstatus_{DATA.id}">
									<!-- BEGIN: sendsuccess -->
									<em class="fa fa-check-circle fa-lg text-success">&nbsp;</em>
									<!-- END: sendsuccess -->
									<!-- BEGIN: senderror -->
									<em class="fa fa-exclamation-circle fa-lg text-danger">&nbsp;</em>
									<!-- END: senderror -->
									{DATA.sendstatus_str}
								</td>
							</tr>
...
<script>
	...
	function nv_sendmail_action(id, customerid) {
		...
		$.ajax({
			url : script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=sendsms&nocache=' + new Date().getTime(),
			type : 'post',
			data : 'send=1&id=' + id,
			dataType : 'json',
			beforeSend : function() {
