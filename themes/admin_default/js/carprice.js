...
function nv_main_action(oForm, checkss, msgnocheck) {
    ...
    if (listid != '') {
        ...
        if (action == 'delete') {
            if (confirm(nv_is_del_confirm[0])) {
                $.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=product&nocache=' + new Date().getTime(), 'deleteselect=1&listid=' + listid + '&checkss=' + checkss, function(res) {
