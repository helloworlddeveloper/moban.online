...
{
    global $home, $nv_Request, $array_mod_title, $lang_global, $language_array, $global_config, $site_mods, $module_name, $module_info, $op_file, $mod_title, $my_head, $my_footer, $client_info, $module_config, $op, $drag_block;
    ...
    if( $global_config['site_phone'] != '' ){
        $site_phone = $global_config['site_phone'];
        $hotline = '0981.565.675';
    }else{
        $site_phone = '0869.236.236';
        $hotline = '0981.565.675';
    }
    $mobile = $nv_Request->get_title('affiliate_mobile', 'session' );
    if( $mobile != '' ){
        $site_phone = $mobile;
        $hotline = $mobile;
    }
    $xtpl->assign('site_phone', $site_phone);
    $xtpl->assign('hotline', $hotline);
    // System variables
