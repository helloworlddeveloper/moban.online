				...
				<tr>
					<th>
						...
						<select name="tranfer_from" onchange="nv_get_info_numproduct(this.value)" class="form-control">
							...
							<!-- BEGIN: loop -->
							<option value="{DEPOT.userid}">{DEPOT.code} {DEPOT.fullname}</option>
							<!-- END: loop -->
						</select>
						<!-- END: tranfer_from -->
					</th>
	...
	function nv_get_info_numproduct( depotid ) {
	    if( depotid > 0 ){
            $.ajax({
                type : 'POST',
                dataType: 'json',
                url : nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=tranfer',
                data : 'loaddepot=1&depotid=' + depotid,
    ...
    function send_data_tranfer() {
		...
		$('.productid_number').each(function () {
		    if( $(this).val() == '' ){
                ...
                submit = 0;
		        return false;
            }else if( $(this).val() > parseInt(  $('#productid_' + $(this).attr('data_productid')).html() ) ){
                $(this).focus();
                $(this).addClass('error');
                submit = 0
                return false;
		...
		if( submit == 1 ){
            $.ajax({
                type : 'POST',
                url : nv_base_siteurl + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=tranfer',
                ...
                success : function(data) {
                    ...
                    if( data[0] == 'ERROR'){
                        ...
                        alert('Luân chuyển hàng hóa thành công')
                        window.location.href = nv_base_siteurl + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=warehouse_logs';
                    }
                }
            });
        }
