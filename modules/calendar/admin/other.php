...
if( $nv_Request->isset_request( 'delete_id', 'get' ) and $nv_Request->isset_request( 'delete_checkss', 'get' ) )
{
    ...
    {
        $db->query( 'DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_other WHERE id = ' . $db->quote( $id ) );
...
if( $nv_Request->isset_request( 'checktime', 'post' ) ) {
    ...
    $sql = 'SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_other' . $sql_check;
...
elseif( $row['id'] > 0 )
{
    $row = $db->query( 'SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data .'_other WHERE id=' . $row['id'] )->fetch();
    if( empty( $row ) )
    {
        Header( 'Location: ' . NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op );
        die();
...
else
{
    ...
    $db->sqlreset()->select( 'COUNT(*)' )->from( NV_PREFIXLANG . '_' . $module_data . '_other' );
