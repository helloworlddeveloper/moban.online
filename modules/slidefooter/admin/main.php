...
if ( $nv_Request->isset_request( 'submit', 'post' ) )
{
	...
	if( empty( $error ) )
	{
		try
		{
			if( empty( $row['id'] ) )
			{
				$stmt = $db->prepare( 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . ' (title, alias, link, image, bodytext, status) VALUES (:title, :alias, :link, :image, :bodytext, :status)' );
			}
			else
			{
				$stmt = $db->prepare( 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . ' SET title = :title, alias = :alias, link = :link, image = :image, bodytext = :bodytext, status = :status WHERE id=' . $row['id'] );
			}
...
if ( ! $nv_Request->isset_request( 'id', 'post,get' ) )
{
	...
	if( ! empty( $q ) )
	{
		$db->where( 'title LIKE :q_title OR alias LIKE :q_alias OR image LIKE :q_image' );
	}
