...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $db->query('DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_manage_itinerary  WHERE id = ' . $db->quote($id));
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    $row['vehicle'] = $nv_Request->get_title('vehicle', 'post', '');
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
        ...
        $view['link_view'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=detail&amp;id=' . $view['id'];
