 ...
 * @Createdate Fri, 12 Jan 2018 10:26:26 GMT
 */
...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($delcustomer > 0 and $delete_checkss == md5($delcustomer . NV_CACHE_PREFIX . $client_info['session_id'])) {
        ...
        Header('Location: ' . NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op . '&id=' . $row['id'] . '#tab_customer');
        die();
    }
    elseif ($delcommodity > 0 and $delete_checkss == md5($delcommodity . NV_CACHE_PREFIX . $client_info['session_id'])) {
        ...
        $nv_Cache->delMod($module_name);
        Header('Location: ' . NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op . '&id=' . $row['id'] . '#tab_commodity');
        die();
    }
    elseif ($delcost > 0 and $delete_checkss == md5($delcost . NV_CACHE_PREFIX . $client_info['session_id'])) {
        ...
        $nv_Cache->delMod($module_name);
        Header('Location: ' . NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op . '&id=' . $row['id'] . '#tab_cost');
        die();
    }
}
...
$id = $row['id'];
$current_url =  NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op .'&amp;id='.$row['id'];
...
if ( $nv_Request->isset_request( 'submit_customer', 'post' ) )
{
    ...
    if( empty( $error ) )
    {
        try
        {
            ...
            $stmt->bindParam( ':localtion_customer_start', $row['localtion_customer_start'], PDO::PARAM_STR );
            $stmt->bindParam( ':localtion_customer_end', $row['localtion_customer_end'], PDO::PARAM_STR );
...
elseif ( $nv_Request->isset_request( 'submit_cost', 'post' ) )
{
    ...
    $row['localtion_cost'] = $nv_Request->get_title( 'localtion_cost', 'post', 0 );
    ...
    if( empty( $error ) )
    {
        try
        {
            ...
            $stmt->bindParam( ':localtion_cost', $row['localtion_cost'], PDO::PARAM_STR );
...
while ($row = $_query->fetch()) {
    ...
    if( defined('NV_IS_ADMIN')){
        $xtpl->parse( 'main.customer.loop.adminfunction' );
    }
...
while ($row = $_query->fetch()) {
    ...
    if( defined('NV_IS_ADMIN')){
        $xtpl->parse( 'main.commodity.loop.adminfunction' );
    }
...
while ($row = $_query->fetch()) {
    ...
    if( defined('NV_IS_ADMIN')){
        $xtpl->parse( 'main.cost.loop.adminfunction' );
    }
