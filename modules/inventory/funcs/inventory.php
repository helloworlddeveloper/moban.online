...
elseif( $nv_Request->isset_request( 'submit', 'post' ) )
{
    ...
    $data['nguyennhan'] = $nv_Request->get_title( 'nguyennhan', 'post', '');
    $data['kiennghi'] = $nv_Request->get_title( 'kiennghi', 'post', '');
    ...
    elseif( $data['month'] > 12 ){
        ...
        $sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_inventory (
                departmentid, time_inventory, nguyennhan, kiennghi, addtime
            ) VALUES (
                 ' . intval( $data['departmentid'] ) . ',
                 ' . intval( $data['time_inventory'] ) . ',
                 ' . $db->quote( $data['nguyennhan'] ) . ',
                 ' . $db->quote( $data['kiennghi'] ) . ',
                 ' . NV_CURRENTTIME . '
            )';
