...
//kiem tra xem co du quyen k
    ...
    if( !empty( $module_config[$module_name]['headerfile']) and file_exists(NV_UPLOADS_REAL_DIR . '/' . $module_upload . '/' . $module_config[$module_name]['headerfile'])){
        ...
        $objDrawing->setHeight(80); // thumb height
        $objDrawing->setWidth(80); // thumb height
        $objDrawing->setWorksheet($objWorksheet);
    }
    //cong ty
    $objWorksheet->setCellValue( 'B1', '                  ' . $module_config[$module_name]['companyname'] );
    $objWorksheet->setCellValue( 'B2', '                  ' . $module_config[$module_name]['address'] );
    ...
    $sql = 'SELECT t1.*, t2.first_name, t2.last_name FROM ' . NV_PREFIXLANG . '_' . $module_data . '_inventory_users AS t1 INNER JOIN ' . NV_USERS_GLOBALTABLE . ' AS t2 ON t1.userid=t2.userid WHERE iid=' . $id;
    $result = $db->query( $sql );
    ...
    $sql = 'SELECT t2.*, t1.price AS price_conlai, t1.amount as amount_inventory, t1.amount_broken, t1.amount_redundant, t1.amount_missing, t1.note FROM ' . NV_PREFIXLANG . '_' . $module_data . '_inventory_detail AS t1 INNER JOIN ' . NV_PREFIXLANG . '_' . $module_data . '_product AS t2 ON t1.pid=t2.id WHERE iid=' . $id;
    $stt = 1;
    $result = $db->query( $sql );
    $numrow = $result->rowCount();
    ...
    $i = $i + 2;
    //nguyen nhan thuathieu
    $objWorksheet->setCellValue( 'B' . $i, $lang_module['nguyennhanthuathieu'] . ': ' . $data_inventory['nguyennhan'] );
    $i++;
    $objWorksheet->setCellValue( 'B' . $i, $lang_module['kiennghi'] . ': ' . $data_inventory['kiennghi'] );
