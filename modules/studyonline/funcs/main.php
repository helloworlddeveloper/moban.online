...
if(empty($contents))
{
	$db_slave->sqlreset()->select('COUNT(*)')->from(NV_PREFIXLANG . '_' . $module_data . '_khoahoc')->where('status= 1');
	$num_items = $db_slave->query($db_slave->sql())->fetchColumn();

	$db_slave->select('id, classid, subjectid, title, alias, image,hometext,addtime,numview, numlike, price, teacherid, numbuy, listtag')->order('id ASC')->limit($per_page)->offset(($page - 1) * $per_page);
