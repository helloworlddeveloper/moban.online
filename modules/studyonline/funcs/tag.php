...
if(! empty($page_title) and $page_title == strip_punctuation($page_title))
{
    ...
    if($tid > 0)
    {
        ...
        $db_slave->sqlreset()->select('COUNT(*)')->from(NV_PREFIXLANG . '_' . $module_data . '_khoahoc')->where('status=1 AND id IN (SELECT id FROM ' . NV_PREFIXLANG . '_' . $module_data . '_tags_id WHERE tid=' . $tid . ')');
        ...
        $contents = topic_theme($item_array, $generate_page, $page_title, $description, $image_tag);
