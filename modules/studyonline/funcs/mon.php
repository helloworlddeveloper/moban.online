...
$page_title = sprintf( $lang_module['class_title_info'], $array_subject[$subjectid]['title'], $array_class[$classid]['title'] );
...
if (! defined('NV_IS_MODADMIN') and $page < 5) {
    ...
    $cache_file = NV_LANG_DATA . '_' . $module_info['template'] . '_' . $op . '_' . $classid . '_' . $subjectid . '_' . $page . '_' . NV_CACHE_PREFIX . '.cache';
...
if(empty($contents))
{
	$db_slave->sqlreset()->select('COUNT(*)')->from(NV_PREFIXLANG . '_' . $module_data . '_khoahoc')->where('status= 1 AND classid=' . $classid . ' AND subjectid=' . $subjectid);
	...
	$contents = nv_theme_mon_studyonline($array_data, $page_title, $subjectid, $generate_page);
}
if($page > 1)
{
