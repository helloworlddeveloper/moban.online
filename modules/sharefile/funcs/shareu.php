...
$db->sqlreset()->select( 'COUNT(*)' )->from( NV_PREFIXLANG . '_' . $module_data . '_sharing AS s INNER JOIN ' . NV_PREFIXLANG . '_' . $module_data . ' AS f ON s.fileid=f.id' )->where( 's.user_id=' . $user_info['userid'] );

$num_items = $db->query( $db->sql() )->fetchColumn();
$array_item = $array_users = array();
if( $num_items )
{
	$db->select( 'f.id, f.title, f.alias, f.uploadtime, f.filesize, f.download_hits' );
	$db->order( 'f.uploadtime DESC' );
...
$contents = theme_main_share_u( $array_item, $download_config );
