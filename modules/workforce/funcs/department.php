...
$table_name = NV_PREFIXLANG . "_" . $module_data . "_department";
...
if (! empty($savecat)) {
    ...
    if( empty( $data['title'])){
        $error = $lang_module['department_name_error'];
    }elseif($data['userid'] == 0){
        $error = $lang_module['department_userid_error'];
    }else{
        if ($data['id'] == 0) {

            $sql = "INSERT INTO " . $table_name . " (id, userid, title, note) VALUES (NULL, " . $data['userid']. ',' . $db->quote( $data['title'] ) . ", " . $db->quote( $data['note'] ) . ")";
            ...
            $stmt = $db->prepare("UPDATE " . $table_name . " SET userid=" . $data['userid'] .", title= :title, note = :note WHERE id =" . $data['id']);
...
if( $add == 0 && $data['id'] == 0 ){
    ...
    $sql = 'SELECT  userid, username, first_name, last_name, birthday, email FROM ' . NV_USERS_GLOBALTABLE . ' WHERE active=1';

    $result = $db_slave->query($sql);
    while ($row = $result->fetch()) {
        $row['fullname'] = nv_show_name_user( $row['first_name'], $row['last_name'], $row['username'] );
        $array_users_list[$row['userid']] = $row;
    }

    foreach ($array_users_list as $userid_i => $user) {
        $user['sl'] = ($userid_i == $data['userid']) ? ' selected="selected"' : '';
        $xtpl->assign('CAT_SUB', $user  );
        $xtpl->parse('main.add.catinfo');
    }
    $xtpl->parse('main.add');
}
