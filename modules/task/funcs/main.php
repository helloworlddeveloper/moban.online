...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id']) && nv_check_task_admin($useradd)) {
        nv_task_delete($id);
        nv_insert_logs(NV_LANG_DATA, $module_name, $lang_module['main'], $lang_module['logs_delete_task'] . $task_name['title'], $user_info['userid']);
    ...
    if (!empty($array_id)) {
        ...
        nv_insert_logs(NV_LANG_DATA, $module_name, $lang_module['main'], $lang_module['logs_delete_list_task'], $user_info['userid']);
        $nv_Cache->delMod($module_name);
        die('OK');
    }
    die('NO');
...
if ($nv_Request->isset_request('change_read', 'get')) {
    ...
    if ($id > 0) {
        $db->query('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_performer SET readed=0 WHERE taskid=' . $id . ' AND userid=' . $user_info['userid']);
        nv_insert_logs(NV_LANG_DATA, $module_name, $lang_module['main'], sprintf($lang_module['logs_change_read'], $task_name['title']), $user_info['userid']);
        $nv_Cache->delMod($module_name);
        Header('Location: ' . NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
        die();
    }
...
if (!empty($array_search['userid'])) {
    ...
    $where .= ' AND t2.userid=' . $array_search['userid'];
}
...
$db->sqlreset()
    ...
    ->join('INNER JOIN ' . NV_PREFIXLANG . '_' . $module_data . '_performer t2 ON t1.id=t2.taskid')
    ->where('1=1' . $where);
...
$db->select('DISTINCT id, title, performer, begintime, exptime, realtime, description, useradd, addtime, edittime, status, priority, t2.readed')
...
while ($view = $sth->fetch()) {
    ...
    $view['edittime'] = (empty($view['edittime'])) ? '' : nv_date('H:i d/m/Y', $view['edittime']);
    ...
    if ($is_download) {
        ...
        $view['description'] = nv_unhtmlspecialchars($view['description']);
    }
...
if ($is_download && !empty($array_data)) {
    ...
    nv_insert_logs(NV_LANG_DATA, $module_name, $lang_module['main'], $lang_module['logs_download_excel'], $user_info['userid']);
    nv_task_download($array_data);
