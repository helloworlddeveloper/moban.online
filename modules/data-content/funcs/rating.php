...
if ($id > 0 and in_array($point, $array_point) and $checkss == md5($id . NV_CHECK_SESSION)) {
    ...
    $query = $db->query("SELECT total_rating, click_rating FROM " . NV_PREFIXLANG . "_" . $module_data . " WHERE id = " . $id . " AND status=1");
    $row = $query->fetch();
    if ( !empty( $row )) {
        $query = "UPDATE " . NV_PREFIXLANG . "_" . $module_data . " SET total_rating=total_rating+" . $point . ", click_rating=click_rating+1 WHERE id=" . $id;
        $db->query($query);
