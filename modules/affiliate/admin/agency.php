...
if( $nv_Request->isset_request('change_weight', 'post', 0) ){
    ...
    $sql = 'SELECT id FROM ' . $db_config['prefix'] . '_' . $module_data . '_agency WHERE id=' . $id;
    ...
    $sql = 'SELECT id FROM ' . $db_config['prefix'] . '_' . $module_data . '_agency WHERE id!=' . $id . ' ORDER BY weight ASC';
    $result = $db->query($sql);
    ...
    while ($row = $result->fetch()) {
        ...
        $sql = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_agency SET weight=' . $weight . ' WHERE id=' . $row['id'];
        $db->query($sql);
    }

    $sql = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_agency SET weight=' . $new_weight . ' WHERE id=' . $id;
    $db->query($sql);
    ...
    exit();
        ...
        $sql = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_agency SET status=' . $new_status . ' WHERE id=' . $id;
        $db->query($sql);
        $nv_Cache->delMod($module_name);
    ...
    if (md5($id . NV_CHECK_SESSION) == $checkss) {
        ...
        $sql = 'DELETE FROM ' . $db_config['prefix'] . '_' . $module_data . '_agency WHERE id = ' . $id;
        if ($db->exec($sql)) {
            nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete', 'ID: ' . $id, $admin_info['userid']);

            $sql = 'SELECT id FROM ' . $db_config['prefix'] . '_' . $module_data . '_agency ORDER BY weight ASC';
            $result = $db->query($sql);
            $weight = 0;
            while ($row = $result->fetch()) {
                ++$weight;
...
$sql = 'SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_agency ORDER BY weight ASC';
