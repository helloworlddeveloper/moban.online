...
if( $type == ''){
    $db->sqlreset()->select( 'userid, username,first_name, last_name, birthday, email' )->from( NV_USERS_GLOBALTABLE )->where( 'userid NOT IN (SELECT userid FROM ' . $db_config['prefix'] . '_' . $module_data . '_users) AND (username LIKE :username OR concat(last_name," ",first_name) LIKE :fullname OR email LIKE :email)' )->limit( 50 );
}
else{
