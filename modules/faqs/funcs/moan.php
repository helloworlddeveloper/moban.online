...
$sql = 'SELECT SQL_CALC_FOUND_ROWS an.id, an.answer, an.addtime, an.cus_name, an.cus_email, an.file, que.title, que.alias FROM ' . NV_PREFIXLANG . '_' . $module_data . '_answer as an INNER JOIN ' . NV_PREFIXLANG . '_' . $module_data . '_question as que ON an.id = que.most WHERE an.status=1 AND an.userid = ' . $user_info['userid'] . ' ORDER BY an.addtime DESC LIMIT ' . $page . ', ' . $per_page;
$result = $db->query( $sql );
	...
	foreach( $array as $k => $v )
	{
		if( $v['file'] != '' )
		{
			...
			if( file_exists( NV_ROOTDIR . '/' . $file2 ) and ( $filesize = filesize( NV_ROOTDIR . '/' . $file2 ) ) != 0 )
			{
				...
				$session_files['fileupload'][$new_name] = array( 'qid' => $v['id'], 'src' => NV_ROOTDIR . '/' . $file2 );
			}
