...
$a = array();
$s = "SELECT qid, alias, title FROM " . NV_PREFIXLANG . "_" . $module_data . "_question WHERE status != 0 ORDER BY addtime DESC";
$re = $db->query( $s );
while( $r = $re->fetch() )
{
	$a[$r['qid']] = array( 'alias' => $r['alias'], 'title' => $r['title'] );
}

$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM " . NV_PREFIXLANG . "_" . $module_data . "_answer WHERE status=1 AND userid = " . $user_info['userid'] . " ORDER BY addtime DESC LIMIT " . $page . ", " . $per_page;
$result = $db->query( $sql );
