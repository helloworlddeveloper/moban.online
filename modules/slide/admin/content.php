...
$page_title = $lang_module['album_add_link'];
...
if ($data['id'] > 0) {
    ...
    $page_title = $lang_module['album_edit_link'];
}
...
if ($nv_Request->get_int('save', 'post,get', 0)) {
    ...
    if (empty($error)) {
        if ($data['id'] > 0) {
            ...
            if ($stmt->execute()) {
                nv_insert_logs(NV_LANG_DATA, $module_name, $lang_module['album_edit_link'], $data['title'], $admin_info['userid']);
            ...
            $stmt->bindParam(':description', $data['description'], PDO::PARAM_STR, strlen($data['description']));
            if ($stmt->execute()) {
                nv_insert_logs(NV_LANG_DATA, $module_name, $lang_module['album_add_link'], $data['title'], $admin_info['userid']);
