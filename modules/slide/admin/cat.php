...
if (! empty($savecat)) {
    ...
    if (empty($data_content['title'])) {
        $error = $lang_module['album_sub_input'];
    } else {
...
if (! empty($array_cat)) {
    foreach ($array_cat as $cat) {
        ...
        if ($cat['parentid'] == $data_content['parentid']) {
            ...
            $cat['inhome_select'] = drawselect_yesno($select_name = 'slinhome', $cat['inhome'], $lang_module['album_no'], $lang_module['album_yes'], 'nv_chang_cat(this,' . $cat['catid'] . ',\'inhome\');');
            $xtpl->assign('ROW', $cat);
