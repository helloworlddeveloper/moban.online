...
if (!nv_function_exists('nv_slide_produce')) {
    /**
     * nv_block_config_page_produce()
     *
     * @param string $module
     ...
     * @return string
     */
    function nv_block_config_page_produce($module, $data_block, $lang_block)
    {
    ...
    /**
     * nv_block_config_page_produce_submit()
     *
     * @param string $module
     * @param array  $lang_block
     * @return array
     */
    function nv_block_config_page_produce_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = [];
        $return['error'] = [];
     ...
     * @return string
     */
    function nv_slide_produce($block_config)
    {
        ...
        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/slide/block_produce.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/slide/block_produce.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('block_produce.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/slide');
			$xtpl->assign('THEMES_URL', NV_BASE_SITEURL. '/themes/' . $block_theme.'/css/slide');
			$xtpl->assign('detail', $lang_module['detail']);
			$xtpl->assign('bg', NV_BASE_SITEURL.'themes/' . $global_config['module_theme'] . '/assets1/img/banner-chinh.jpg');
			$p=1;$t=1;
            foreach ($list as $l) {
...
if (defined('NV_SYSTEM')) {
    $content = nv_slide_produce($block_config);
}
