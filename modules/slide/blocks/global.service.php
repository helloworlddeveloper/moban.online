...
if (!nv_function_exists('nv_slide_service')) {
    /**
     * nv_block_config_service_list()
     *
     * @param string $module
     ...
     * @return string
     */
    function nv_block_config_service_list($module, $data_block, $lang_block)
    {
    ...
    /**
     * nv_block_config_service_list_submit()
     *
     * @param string $module
     * @param array  $lang_block
     * @return array
     */
    function nv_block_config_service_list_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = [];
        $return['error'] = [];
     ...
     * @return string
     */
    function nv_slide_service($block_config)
    {
        ...
        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/slide/block_service.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/slide/block_service.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('block_service.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/slide');
			$xtpl->assign('THEMES_URL', NV_BASE_SITEURL. '/themes/' . $block_theme.'/css/slide');
			$xtpl->assign('bg', NV_BASE_SITEURL. 'themes/' . $block_theme.'/assets1/img/nen-mln2.jpg');
			$stt=0;
            foreach ($list as $l) {
                $l['url_img'] = NV_BASE_SITEURL . 'uploads/' . $l['urlimg'];
				$l['active']=$stt==0?" active ":"";
				$l['stt']=++$stt;
				$l['noidung']=!empty($l['description'])?$l['description']:$l['title'];
				$xtpl->assign('ROW', $l);
                $xtpl->parse('main.row');
            }
            $xtpl->parse('main');
...
if (defined('NV_SYSTEM')) {
    $content = nv_slide_service($block_config);
}
