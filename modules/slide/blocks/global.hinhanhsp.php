...
if (!nv_function_exists('nv_slide_hinhanhsp')) {
    /**
     * nv_block_config_page_hinhanhsp()
     *
     * @param string $module
     ...
     * @return string
     */
    function nv_block_config_page_hinhanhsp($module, $data_block, $lang_block)
    {
    ...
    /**
     * nv_block_config_page_hinhanhsp_submit()
     *
     * @param string $module
     * @param array  $lang_block
     * @return array
     */
    function nv_block_config_page_hinhanhsp_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = [];
        $return['error'] = [];
     ...
     * @return string
     */
    function nv_slide_hinhanhsp($block_config)
    {
        ...
        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/slide/block_hinhanhsp.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/slide/block_hinhanhsp.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('block_hinhanhsp.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/slide');
            ...
            foreach ($list as $l) {
				...
				$xtpl->assign('ROW', $l);
                $xtpl->parse('main.row.loop'); $p++;
				if ($p%4==0) $xtpl->parse('main.row');
            }
			if ($p%4>0) $xtpl->parse('main.row');
            $xtpl->parse('main');
...
if (defined('NV_SYSTEM')) {
    $content = nv_slide_hinhanhsp($block_config);
}
