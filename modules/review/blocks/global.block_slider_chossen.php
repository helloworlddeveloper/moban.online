...
if (!nv_function_exists('nv_global_bxslider_center')) {
    ...
    function nv_block_config_bxslider_center_blocks($module, $data_block, $lang_block)
    {
        ...
        $html .= '<div class="col-sm-18"><select name="config_template" class="form-control">';
        foreach ($array_template as $key => $value) {
            $html .= '<option value="' . $key . '" ' . ($data_block['config_template'] == $key ? 'selected="selected"' : '') . '>' . $value . '</option>';
        }
    ...
    function nv_block_config_bxslider_center_blocks_submit($module, $lang_block)
    {
        ...
        $return['config']['template'] = $nv_Request->get_title('config_template', 'post', 'caroulsel');
        return $return;
    }
    ...
    function nv_global_bxslider_center($block_config)
    {
        ...
        if (file_exists(NV_ROOTDIR . "/themes/" . $global_config['module_theme'] . "/modules/" . $mod_file . "/block.slider_" .$block_config['template'] . ".tpl")) {
        ...
        $xtpl = new XTemplate('block.slider_' .$block_config['template'] . '.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod_file);
        $xtpl->assign('LANG', $lang_module);
        $xtpl->assign('CAT', $list_cat[$block_config['catid']]);
        $xtpl->assign('THEME_TEM', NV_BASE_SITEURL . 'themes/' . $block_theme);
        $xtpl->assign('LIB_PATH', NV_BASE_SITEURL . 'modules/' . $module . '/libs');
        ...
        foreach ($list as $row) {
            ...
            if( $block_config['template'] == 'slick'){
                $stt++;
                if( $stt == 1){
                    $xtpl->assign('class', 'content__ally__item__info');
                }
                $xtpl->parse('main.items.loop');
                if( $stt %2 == 0 ){
                    $class = 'content__ally__item__info';
                    $xtpl->parse('main.items');
                }else{
                    $class = 'content__ally__item__info_top';
                }
        ...
        if (!defined('NV_LIBS_' . $block_config['template'])) {
            define('NV_LIBS_' . $block_config['template'], true);
            $xtpl->parse('main.lib');
        }
