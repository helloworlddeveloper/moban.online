...
if ( $nv_Request->isset_request( 'submit', 'post' ) )
{
    ...
    $row['jobs'] = $nv_Request->get_title( 'jobs', 'post', '' );
	$row['link'] = $nv_Request->get_title( 'link', 'post', '' );
	$row['image'] = $nv_Request->get_title( 'image', 'post', '' );
    $row['bodytext'] = $nv_Request->get_textarea('bodytext', '');
	...
	if( empty( $error ) )
	{
		try
		{
			if( empty( $row['id'] ) )
			{
				$stmt = $db->prepare( 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . ' (title, bodytext, jobs, catid, link, image, status) VALUES (:title, :bodytext, :jobs, :catid, :link, :image, :status)' );
			}
			else
			{
				$stmt = $db->prepare( 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . ' SET catid =:catid, title = :title, bodytext=:bodytext, jobs=:jobs, link = :link, image = :image, status = :status WHERE id=' . $row['id'] );
			}
            $stmt->bindParam( ':catid', $row['catid'], PDO::PARAM_INT );
            $stmt->bindParam(':bodytext', $row['bodytext'], PDO::PARAM_STR, strlen($row['bodytext']));
			$stmt->bindParam( ':jobs', $row['jobs'], PDO::PARAM_STR );
...
{
    $cat['sl'] = ($row['catid'] == $cat['id'])? ' selected=selected' : '';
