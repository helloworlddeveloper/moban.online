...
if ($nv_Request->isset_request('get_keyword', 'post')) {
    foreach( $site_mods as $mod_name => $site_info ){
        if( file_exists( NV_ROOTDIR . '/modules/' . $site_info['module_file'] . '/keyword.php')){
            $mod_data = $site_info['module_data'];
            include NV_ROOTDIR . '/modules/' . $site_info['module_file'] . '/keyword.php';
            //ghi cac tu khoa vao he thong
            foreach( $array_content_keyword as $keyword ){
                $sth = $db->prepare('INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_tags (numsearch, module_name, alias, description, image, keywords) VALUES (0, :module_name, :alias, :description, :image, :keywords)');
                try {
                    $alias = html_entity_decode( $keyword );
    ...
    $db_slave->sqlreset()->select('module_name, keywords')->from(NV_PREFIXLANG . '_' . $module_data . '_tags')->order('module_name, alias ASC');

    $sth = $db_slave->prepare($db_slave->sql());
    $sth->execute();

    $xtpl = new XTemplate('tags_return.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
    $xtpl->assign('display', '{display}');
    $array_keyword_content = array();
    while (list( $mod_name, $title ) = $sth->fetch(3)) {
        $array_keyword_content[$mod_name][] = $title;
    }
    foreach( $array_keyword_content as $mod_name => $mod_name_data ){
    
        $custom_title = $site_mods[$mod_name]['custom_title'];
        foreach( $mod_name_data as $keyword ){
            $xtpl->assign('KEYWORD', $keyword);
            $xtpl->parse('main.module.loop');    
        }
        $xtpl->assign('custom_title', $custom_title);
        $xtpl->assign('module_name', str_replace('-', '_', $mod_name));
        $xtpl->parse('main.module');
        $xtpl->parse('main.module_show');
    }
    $xtpl->parse('main');
    $contents = $xtpl->text('main');
    file_put_contents( NV_ROOTDIR . '/modules/' . $module_file . '/js/data.js', $contents);
...
if (! empty($savecat)) {
    ...
    if (empty($alias)) {
        ...
        if ($tid == 0) {
            $sth = $db->prepare('INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_tags (numsearch, alias, description, image, keywords) VALUES (0, :alias, :description, :image, :keywords)');
            $msg_lg = 'add_tags';
        } else {
