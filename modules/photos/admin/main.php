								...
								if( file_exists($newFilePath) OR $rename )
								{
									...
									else
									{
										...
										$sth = $db->prepare( 'INSERT INTO ' . TABLE_PHOTO_NAME . '_rows SET 
											...
											author=' . intval( $admin_info['admin_id'] ) . ',
											download=0,
											name = :name,
											description = :description,
											file = :file,
...
if( ! empty( $array ) )
{
	foreach( $array as $item )
	{
		...
		$item['link'] = NV_BASE_ADMINURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=view&token=" . $item['token'] . "&album_id=" . $item['album_id'];
