...
if( $album_id > 0 and in_array( $point, $array_point ) and $checkss == md5( $album_id . $client_info['session_id'] . $global_config['sitekey'] ) )
{
	...
	$query = $db->query( "SELECT  total_rating, click_rating FROM " . TABLE_PHOTO_NAME . "_album WHERE album_id = " . $album_id . " AND status=1" );
	$row = $query->fetch();
	$query = "UPDATE " . TABLE_PHOTO_NAME . "_album SET total_rating=total_rating+" . $point . ", click_rating=click_rating+1 WHERE album_id=" . $album_id;
	$db->query( $query );
