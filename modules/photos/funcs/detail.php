...
{	
	...
	//Next Photo
	$sql = 'SELECT row_id, album_id, name, status, description FROM ' . TABLE_PHOTO_NAME . '_rows WHERE row_id > '.$row['row_id'].' AND album_id='.$row['album_id'].' ORDER BY row_id ASC LIMIT 1';
	...
	//Previous Photo
	$sql = 'SELECT row_id, album_id, name, status, description FROM ' . TABLE_PHOTO_NAME . '_rows WHERE row_id < '.$row['row_id'].' AND album_id='.$row['album_id'].' ORDER BY row_id DESC LIMIT 1';
	$list = $nv_Cache->db( $sql, 'row_id', $module_name );
