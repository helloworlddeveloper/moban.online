...
function detail_album($album, $array_photo, $other_category_album, $content_comment, $generate_page = '', $page = 1)
{
    ...
    $per_line = 24 / $module_config[$module_name]['per_line'];
    $xtpl->assign('PER_LINE', $per_line);
    $xtpl->assign('add_more_photo', NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=uppic&action=edit&token=" . md5( $global_config['sitekey'] . session_id() . $album['album_id'] ) . "&album_id=" . $album['album_id']);
    if (! empty($album)) {
		...
		$link_down = NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . '=down';
        if (! empty($array_photo)) {
            foreach ($array_photo as $photo) {
                ...
                $photo['download_image'] = $link_down . '&row_id=' . $photo['row_id'] . '&token=' . md5( NV_CHECK_SESSION . $photo['row_id'] );
