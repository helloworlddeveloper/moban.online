...
else
{
    ...
    $customertype = $nv_Request->get_int( 'customertype', 'post,get', 1 );
	$xtpl->assign( 'Q', $q );
	if( $provinceid > 0 )
	{
    ...
    $array_customtype = array(0 => $lang_module['customertype_search_0'],1 => $lang_module['customertype_search_1']);
    foreach( $array_customtype as $key => $title )
    {
        $selected = ( $key == $customertype ) ? ' selected="selected"' : '';
        $xtpl->assign( 'OPTION', array(
            'key' => $key,
            'title' => $title,
            'selected' => $selected ) );
        $xtpl->parse( 'main.view.customertype_select' );
    }
	...
	if( $customertype >=0){
	    if( $customertype > 0 ){
            $sql_where .= ( ! empty( $sql_where ) ) ? ' AND adminid=0' : 'adminid=0';
        }else{
            $sql_where .= ( ! empty( $sql_where ) ) ? ' AND adminid!=0' : 'adminid!=0';
        }
        $base_url .= '&customertype=' . $customertype;
    }
	$db->where( $sql_where );
	...
	while( $view = $sth->fetch() )
	{
		...
		if( $permissions_users[$admin_info['admin_id']]['eventcontent']['view'] == 1 )
		{
	...
	if( $permissions_users[$admin_info['admin_id']][$op]['add'] == 1 )
	{
		$xtpl->parse( 'main.view.allow_link_add' );
	}
	if( $permissions_users[$admin_info['admin_id']]['viewstudent']['view'] == 1 )
	{
		$xtpl->parse( 'main.view.allow_link_viewstudent_view' );
	}
