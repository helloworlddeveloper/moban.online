...
if (!empty($id)) {
    $rows = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_smsrows WHERE id=' . $id)->fetch();
    if ($rows) {
        ...
        if ($nv_Request->isset_request('send', 'post')) {
            if ($module_config[$module_name]['sms_on'] == 0) {
            ...
            if (!empty($array_send)) {
                foreach ($array_send as $index => $customerid) {
                    ...
                    if ($result) {
                        ...
                        die(nv_sendphone_action($customer, $rows));
                    }
                }
            } else {
                die(json_encode(array(
        ...
        if ($nv_Request->isset_request('linkstatics', 'post,get')) {
            $array_link = array();
            $result = $db->query('SELECT countclick, listclick FROM ' . NV_PREFIXLANG . '_' . $module_data . '_smsrows_link WHERE rowsid=' . $id);
            $i = 0;
