...
if( $step == 1 )
{
    ...
    $array_user = array();
    $sql = 'SELECT * FROM ' . NV_USERS_GLOBALTABLE . ' WHERE userid IN(' . $subcatid . ')';
    $res = $db->query( $sql );
    ...
    if( !empty( $listall ) ){
        ...
        $sql_show = $nv_Request->get_title( 'sql_show', 'get,post', '' );
    ...
    while ($data = $res->fetch()){
        ...
        while ($row = $result->fetch()) {
            $array_data[$data['user_id']][$data['order_id']]['detail'][$row['proid']] = $row['num'];
        }
    }
