...
if($user_data_affiliate['possitonid'] > 0){
    //cua nhan vien GM WHITE
    $list_customerid = nvGetUseridInParent($user_data_affiliate['userid'], $user_data_affiliate['subcatid'], $checksub = false);
}
else{
    //cua HE THONG PHAN PHOI
    $list_customerid = nvGetUseridInParent($user_data_affiliate['userid'], $user_data_affiliate['subcatid'], $checksub = false, false);
}
if ( !empty( $list_customerid )){
    $where_customer .= '(customer_id  IN (' . implode(',', $list_customerid ) . ')';
}
//lay danh sach cac khach hang le cho minh gioi thieu
$list_customerid = nvGetUseridCustomer();
if ( !empty( $list_customerid )){
    $where_customer .= ' OR ( customer_id IN (' . implode(',', $list_customerid ) . ') AND chossentype=3)';
}
$where_customer .= ')';
// Fetch Limit
...
$db->select('*')->where($where_customer . $where)->order('order_id DESC')->limit($per_page)->offset(($page - 1) * $per_page);
$query = $db->query($db->sql());
while ($row = $query->fetch()) {
    $acno = 0;
    $price = $row['order_total'];
    if (( $row['customer_id'] != $user_info['userid'] ) or ( $row['chossentype'] == 3 && $row['customer_id'] == $user_info['userid'] )) {
