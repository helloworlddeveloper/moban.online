...
if( $nv_Request->isset_request('loadcart', 'get') )
{
    ...
    */
    ...
    $contents = cart_product_load_make_order($data_content, $bonus_cumulative);
    include NV_ROOTDIR . '/includes/header.php';
    echo $contents;
    include NV_ROOTDIR . '/includes/footer.php';
    exit;
...
if( $nv_Request->isset_request('submit', 'post') ) {
    ...
    */
    ...
    $data_order['chossentype'] = 3;//khách lẻ//$nv_Request->get_int('chossentype', 'post', 0);
...
//print_r($data_order);die;
    ...
    //kiem tra ton kho co du xuat khong
    foreach ( $_SESSION[$module_data . '_cart'] as $pro_id => $order_product ) {
        if( intval( $pro_id ) > 0 ) {
            $total_num_product = checkNumTotalInParentCustomer($data_order['customer_id'], $data_order['depotid'], $pro_id, 0);
            $total_number = $order_product['num'] + $order_product['numgif'];
            if ($total_num_product < $total_number && $data_order['ordertype'] == 1) {
                ...
                $result = $db->query( $sql );
                $title = "";
                if ($data_row = $result->fetch( ))
                    $title = $data_row['title'];
                $error['number_in_warehouse'] = sprintf($lang_module['number_in_warehouse_logs_error'], $title, $total_num_product);
            }
        }
    }
    ...
    if( $data_order['customer_id'] == 0 && !empty( $data_order['order_phone'] ) && !empty( $data_order['order_name'] ) && empty( $error ) ){
        ...
        $result_return = createUserCustomer( 3, $data_order['order_name'], $data_order['order_email'], $data_order['order_phone'], $data_order['order_address'], $data_order['agencyid'], $user_info['username'] );
    ...
    if (empty($error)) {
        ...
        if( $data_order['orderid']  == 0 ){
            ...
            $sql = "SELECT precode FROM " . NV_TABLE_AFFILIATE . "_users WHERE userid=" . $user_info['userid'];
            $result = $db->query($sql);
            ...
            $sql = "INSERT INTO " . NV_PREFIXLANG . "_" . $module_data . "_orders (
                ...
                " . $user_info['userid'] . ", 0," . doubleval($data_order['order_total']) . ",
...
$contents = call_user_func( 'cart_product_make_order', $data_order, $user_data_affiliate, $error );
