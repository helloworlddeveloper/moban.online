...
$page_title = $lang_module['report_sale_pro'];
...
$xtpl = new XTemplate("rpt_sale_pro.tpl", NV_ROOTDIR . "/themes/" . $global_config['module_theme'] . "/modules/" . $module_file);
...
$sql = 'SELECT t3.*, t4.title AS title_product, t4.code ';
$sql .= 'FROM (SELECT t2.proid, sum(t2.num) as product, t2.num*t2.price as total ';
$sql .= '		FROM nv4_vi_sm_orders t1 ';
$sql .= '			INNER JOIN nv4_vi_sm_orders_id t2 ON t1.order_id = t2.order_id ';
$sql .= '		WHERE t1.user_id > 0 ';
$sql .= $where;
$sql .= '		GROUP BY t2.proid) as t3 ';
$sql .= '	INNER JOIN nv4_vi_sm_product t4 ON t3.proid=t4.id ';
...
while ($row = $query->fetch()) {
    ...
    if ($row['product'] > 0) {
        $proc = ceil(($row['product'] / 1000) * 100);
