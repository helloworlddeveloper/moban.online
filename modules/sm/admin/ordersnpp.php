...
if( !empty( $subcatid )){
    ...
    //don hang dat truoc
    $sql_where = 'ordertype=1 AND user_id IN (' . $subcatid . ',3210,3212,3214,3218)';
    ...
    $db->select('*')->where($sql_where . $where)->order('order_id DESC');
    $sql_export = $db->sql();
    ...
    $xtpl->assign('sql_show', base64_encode($sql_export));
