...
if ($nv_Request->isset_request('delete_did', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($did > 0 and $delete_checkss == md5($did . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $db->query('DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_bonus_cumulative_indirect  WHERE id = ' . $db->quote($did));
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (empty($error)) {
        try {
            if (empty($row['id'])) {
                ...
                $stmt = $db->prepare('INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_bonus_cumulative_indirect (agencyid, productid, add_time, begin_quantity, end_quantity, percent, discount, bonus_productid, bonus_productnum) VALUES ( :agencyid, :productid, :add_time, :begin_quantity, :end_quantity, :percent, :discount, :bonus_productid, :bonus_productnum)');

                $stmt->bindParam(':add_time', $row['add_time'], PDO::PARAM_INT);
            } else {
                $stmt = $db->prepare('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_bonus_cumulative_indirect SET agencyid = :agencyid, productid = :productid, begin_quantity = :begin_quantity, end_quantity = :end_quantity, percent = :percent, discount = :discount, bonus_productid = :bonus_productid, bonus_productnum = :bonus_productnum WHERE id=' . $row['id']);
            }
    ...
    $row = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_bonus_cumulative_indirect WHERE id=' . $row['id'])->fetch();
    if (empty($row)) {
        Header('Location: ' . NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
        die();
    }
...
if (!$nv_Request->isset_request('id', 'post,get')) {
    ...
    $db->sqlreset()->select('COUNT(*)')->from('' . NV_PREFIXLANG . '_' . $module_data . '_bonus_cumulative_indirect');
    if( $productid > 0 ){
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
        ...
        $view['product'] = $array_product[$view['productid']]['title'];
...
$page_title = $lang_module['bonus_cumulative_indirect'];
