...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (empty($error)) {
        try {
            ...
            if (empty($row['id'])) {
                $data_insert = array();
                $_sql = 'INSERT INTO ' . $table_name . ' (parentid, title, alias, custom_title, keywords, description, description_html, groups_view, image, weight) VALUES (:parentid, :title, :alias, :custom_title, :keywords, :description, :description_html, :groups_view, :image, :weight)';
...
$array_viewtype = array(
    1 => $lang_module['config_indexfile_1']
);
