...
if (! empty($savecat)) {
    ...
    if ($data['id'] == 0) {
        $sql = "INSERT INTO " . $table_name . " (id, salesfrom, salesto, bonus_money) VALUES (NULL, " . floatval( $data['salesfrom'] ) . ",  " . floatval( $data['salesto'] ) . ",  " . floatval( $data['bonus_money'] ) . ")";
        ...
        $stmt = $db->prepare("UPDATE " . $table_name . " SET salesfrom= :salesfrom, salesto = :salesto, bonus_money = :bonus_money WHERE id =" . $data['id']);
