...
$page_title = $lang_module['report_debt'];
...
$xtpl = new XTemplate("rpt_debt.tpl", NV_ROOTDIR . "/themes/" . $global_config['module_theme'] . "/modules/" . $module_file);
...
$sql = 'SELECT t1.*, concat(t2.first_name, " ", t2.last_name, " (", t2.username, ")") as partner ';
$sql .= 'FROM (SELECT user_id as customer, sum(order_total) as price, sum(saleoff) as discount, sum(price_payment) as payment, 1 as type ';
$sql .= '		FROM nv4_vi_sm_orders ';
$sql .= '		WHERE order_total != price_payment and user_id > 0 ';
$sql .= $where;
$sql .= '		GROUP BY user_id) as t1 ';
$sql .= '	LEFT OUTER JOIN nv4_users t2 ON t1.customer=t2.userid  ';
$sql .= 'UNION ALL ';
$sql .= 'SELECT t1.*, concat(t2.fullname, " (", t2.phone, ")") as partner ';
$sql .= 'FROM (SELECT customer_id as customer, sum(order_total) as price, sum(saleoff) as discount, sum(price_payment) as payment, 0 as type ';
$sql .= '		FROM nv4_vi_sm_orders ';
$sql .= '		WHERE order_total != price_payment and user_id = 0 ';
$sql .= $where;
$sql .= '		GROUP BY customer_id) as t1 ';
$sql .= '	LEFT OUTER JOIN nv4_vi_sm_customer t2 ON t1.customer=t2.customer_id ';
...
while ($row = $query->fetch()) {
    $num_items++;
    if ($row['type'] == 0 ) {
        $row['customer_type'] = 'Khách lẻ';
    } else {
        $row['customer_type'] = 'Đại lý/CTV';
    }
