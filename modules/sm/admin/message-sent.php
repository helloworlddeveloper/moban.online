...
if($nv_Request->isset_request('checkstatus', 'post'))
{
    ...
    if( $obj['CodeResponse'] == '100'){
        ...
        $db->query('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_message_history SET status = ' . intval( $obj['SendStatus'] ) . ' WHERE id =' . $id );
    }
...
if (!$nv_Request->isset_request('id', 'post,get')) {
    ...
    $db->sqlreset()->select('COUNT(*)')->from('' . NV_PREFIXLANG . '_' . $module_data . '_message_history');
    if( $productid > 0 ){
