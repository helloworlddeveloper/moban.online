...
if (! function_exists('nv_product_center')) {
    ...
    function nv_product_center($block_config)
    {
        ...
        if (($cache = $nv_Cache->getItem($module_name, $cache_file)) != false) {
            ...
            $db->sqlreset()
                ->select('t1.id, t1.catid, t1.listcatid, t1.' . NV_LANG_DATA . '_title AS title, t1.' . NV_LANG_DATA . '_alias AS alias, t1.homeimgfile, t1.homeimgthumb , t1.homeimgalt, t1.showprice, t1.discount_id')
        ...
        foreach ($array as $row) {
            $link = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $global_array_shops_cat[$row['catid']]['alias'] . '/' . $row['alias'] . $global_config['rewrite_exturl'];
