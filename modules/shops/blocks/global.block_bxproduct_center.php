...
if (!nv_function_exists('nv_global_bxproduct_center')) {
    ...
    function nv_global_bxproduct_center($block_config)
    {
        ...
        if ($module != $module_name) {
            ...
            $my_head .= '<script src="' . NV_BASE_SITEURL . 'themes/' . $block_css . '/js/' . $mod_file . '.js' . '" /></script>';
        ...
        foreach ($list as $row) {
            ...
            $xtpl->assign('LINK', $domain_view . $link);
            $xtpl->assign('TITLE', $row['title']);
            $xtpl->assign('TITLE0', nv_clean60($row['title'], 60));
