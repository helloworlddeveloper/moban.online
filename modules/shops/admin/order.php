...
$db->sqlreset()->select('COUNT(*)')
    ...
    ->join('LEFT JOIN ' . NV_USERS_GLOBALTABLE . ' AS t2 ON t1.user_id=t2.userid')
    ->where('1=1 ' . $where);

$num_items = $db->query($db->sql())->fetchColumn();
$order_info['num_items'] = $num_items;

$db->select('t1.*, t2.username, t2.first_name, t2.last_name')->where('1=1 ' . $where)->order('order_id DESC')->limit($per_page)->offset(($page - 1) * $per_page);
...
while ($row = $query->fetch()) {
    ...
    $row['ref_fullname'] = nv_show_name_user( $row['first_name'], $row['last_name'], $row['username'] );
    if( empty( $row['ref_fullname'] )){
        $row['ref_fullname'] = 'System';
    }
