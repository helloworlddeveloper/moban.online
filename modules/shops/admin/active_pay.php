...
if ($save == 1) {
    /* transaction_status: Trang thai giao dich:
     ...
     5 - Xac nhan dang chuyen hang cho kh
     6 - Xac nhan khach da nhan duoc hang
     */
    ...
    if ($action == 'unpay') {
    ...
    } elseif ($action == 'shiping') {
        $db->query("UPDATE " . $db_config['prefix'] . "_" . $module_data . "_orders SET transaction_status=5 WHERE order_id=" . $order_id);
        nv_insert_logs(NV_LANG_DATA, $module_name, 'Shiping payment product', "Order code: " . $data_content['order_code'], $admin_info['userid']);

        $contents = $lang_module['order_submit_shiping_ok'];

        $nv_Cache->delMod($module_name);
    } elseif ($action == 'shiped') {
        $db->query("UPDATE " . $db_config['prefix'] . "_" . $module_data . "_orders SET transaction_status=6 WHERE order_id=" . $order_id);
        nv_insert_logs(NV_LANG_DATA, $module_name, 'Shiped payment product', "Order code: " . $data_content['order_code'], $admin_info['userid']);

        $contents = $lang_module['order_submit_shiped_ok'];
        ...
        if( isset( $site_mods['affiliate'])){
            ...
            $userid = $data_content['user_id'];//userid gioi thieu don hang
            ...
            $affiliateClass = new moneyService( $userid, $money, $content_tracstion, $mod_name, $product_id );
