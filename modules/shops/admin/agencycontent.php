...
if ($id) {
    $sql = 'SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_agency WHERE id=' . $id;
...
if ($nv_Request->get_int('save', 'post') == '1') {
    ...
    $row['number_sale'] = $nv_Request->get_title('number_sale', 'post',0);
    $row['number_gift'] = $nv_Request->get_title('number_gift', 'post',0);
    $row['percent_sale'] = $nv_Request->get_title('percent_sale', 'post',0);
    ...
    if (empty($row['title'])) {
        ...
        if ($id and !$copy) {
            $_sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_agency SET title = :title, alias = :alias, image = :image, price_require = :price_require, percent_sale=:percent_sale, description = :description, bodytext = :bodytext, keywords = :keywords, edit_time = ' . NV_CURRENTTIME . ' WHERE id =' . $id;
            $publtime = $row['add_time'];
        } else {

            $weight = $db->query("SELECT MAX(weight) FROM " . NV_PREFIXLANG . "_" . $module_data . '_agency')->fetchColumn();
            $weight = intval($weight) + 1;

            $_sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_agency
				(title, alias, image, price_require, percent_sale, description, bodytext, keywords, weight, add_time, edit_time, status) VALUES
				(:title, :alias, :image, :price_require, :percent_sale, :description, :bodytext, :keywords, ' . $weight . ', ' . NV_CURRENTTIME . ', ' . NV_CURRENTTIME . ', 1)';
        ...
        try {
            ...
            $sth->bindParam(':percent_sale', $row['percent_sale'], PDO::PARAM_INT);
