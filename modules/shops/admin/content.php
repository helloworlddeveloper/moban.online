...
if ($rowcontent['id'] > 0) {
    ...
    // Custom fields
    $idtemplate = $db->query('SELECT id FROM ' . $db_config['prefix'] . '_' . $module_data . '_template WHERE alias = "' . preg_replace("/[\_]/", "-", $global_array_shops_cat[$rowcontent['catid']]['form']) . '"')->fetchColumn();
    if ($idtemplate) {
...
if ($nv_Request->get_int('save', 'post') == 1) {
    ...
    $hometext = $nv_Request->get_editor('hometext', '', NV_ALLOWED_HTML_TAGS);
    ...
    $rowcontent['product_price_sale'] = $nv_Request->get_string('product_price_sale', 'post', '');
    $rowcontent['product_price_sale'] = floatval(preg_replace('/[^0-9\.]/', '', $rowcontent['product_price_sale']));
    ...
    $rowcontent['bodytext']  = $nv_Request->get_string('bodytext', '', NV_ALLOWED_HTML_TAGS);
    ...
    $group_cat = array();
    $result = $db->query('SELECT groupid FROM ' . $db_config['prefix'] . '_' . $module_data . '_group_cateid WHERE cateid = ' . $rowcontent['catid']);
    if ($result->rowCount() > 0) {
    ...
    if (empty($error)) {
        ...
        if ($rowcontent['id'] == 0 or $is_copy) {
            ...
            $sql = "INSERT INTO " . $db_config['prefix'] . "_" . $module_data . "_rows (id, catid, listcatid, user_id, addtime, edittime, status, publtime, exptime, archive, product_code, product_number, product_price, product_price_sale, price_config, money_unit, product_unit, product_weight, weight_unit, discount_id, homeimgfile, homeimgthumb, homeimgalt,otherimage,imgposition, copyright, inhome, allowed_comm, allowed_rating, ratingdetail, allowed_send, allowed_print, allowed_save, hitstotal, hitscm, hitslm, showprice " . $listfield . ")
				 VALUES ( NULL ,
            ...
            $data_insert['product_price_sale'] = $rowcontent['product_price_sale'];
            ...
            $stmt->bindParam(':product_price_sale', $rowcontent['product_price_sale'], PDO::PARAM_STR);
...
$rowcontent['product_price_sale'] = number_format($rowcontent['product_price_sale']);
$array_files = array();
if ($pro_config['download_active']) {
...
$listcatid = explode(',', $rowcontent['listcatid']);
foreach ($global_array_shops_cat as $catid_i => $rowscat) {

    $space = intval($rowscat['lev']) * 30;
    $temp = array(
        'catid' => $catid_i,
        'space' => $space,
        'title' => $rowscat['title'],
        'checked' => (in_array($catid_i, $listcatid)) ? ' checked="checked"' : '',
...
$typeprice = ($rowcontent['listcatid']) ? $global_array_shops_cat[$rowcontent['catid']]['typeprice'] : 1;
if ($typeprice == 1) {
    // List discount
