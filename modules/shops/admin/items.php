...
$db->sqlreset()->select('id, catid, listcatid, user_id, homeimgfile, homeimgthumb, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, hitstotal, status, edittime, publtime, exptime, product_number, product_price, product_price_sale, money_unit, product_unit, num_sell, username')->from($from)->order($ord_sql)->limit($per_page)->offset(($page - 1) * $per_page);
$result = $db->query($db->sql());

$theme = $site_mods[$module_name]['theme'] ? $site_mods[$module_name]['theme'] : $global_config['site_theme'];
$a = 0;

while (list($id, $catid, $listcatid, $admin_id, $homeimgfile, $homeimgthumb, $title, $alias, $hitstotal, $status, $edittime, $publtime, $exptime, $product_number, $product_price, $product_price_sale, $money_unit, $product_unit, $num_sell, $username) = $result->fetch(3)) {
    ...
    $xtpl->assign('ROW', array(
        'id' => $id,
        'link' => NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $global_array_shops_cat[$catid]['alias'] . '/' . $alias . $global_config['rewrite_exturl'],
        ...
        'product_price_sale' => nv_number_format($product_price_sale, nv_get_decimals($money_unit)),
