        ...
        foreach ($data_content as $data_row) {
            if ($data_row['num_pro'] > 0) {
                ...
                foreach ($data_row['data'] as $data_row_i) {
                    ...
                    if ( $price['discount_percent'] > 0 and $data_row_i['showprice']) {
...
{
    global $module_info, $lang_module, $module_file, $module_name, $module_upload, $pro_config, $global_config, $global_array_group, $array_wishlist_id, $module_data, $client_info, $global_array_shops_cat, $meta_property, $pro_config, $user_info, $discounts_config, $my_head, $my_footer;
    ...
    if (!empty($data_content)) {
        ...
        $xtpl->assign('CAT_TITLE', $global_array_shops_cat[$data_content['catid']]['title']);
        ...
        if( isset( $price['end_time'] ) && $price['end_time'] > NV_CURRENTTIME ){
            $xtpl->parse('main.discounts_time');
        }
            ...
            if (!empty($data_content['tabs'])) {
                ...
                foreach ($data_content['tabs'] as $tabs_id => $tabs_value) {
                    ...
                    if ($tabs_key == 'content_detail') {
                        ...
                        $tabs_content .= '<div class="fb-comments" data-href="' . $client_info['selfurl'] . '" data-width="100%" data-numposts="5"></div>'; 
                    ...
                    }elseif ($tabs_key == 'content_returnback') {
                        $content_returnback_file = NV_ROOTDIR. '/' . NV_DATADIR . '/' . NV_LANG_DATA . '_' . $module_data . '_returnback_content.txt';
                        if (file_exists( $content_returnback_file)) {
                            $tabs_content = file_get_contents( $content_returnback_file );
                        }
                    }
    ...
    if ($global_array_shops_cat[$data_content['catid']]['typeprice'] == 2) {
 ...
 * @return
 */
function uers_order($data_content, $data_order, $total_coupons, $order_info, $error, $_agency_chossen)
{
    ...
    if (!empty($data_content)) {
        foreach ($data_content as $data_row) {
            ...
            if( !empty( $_agency_chossen )){
                //gia cho agency
                $price_all = nv_get_price_agency( $price['sale'], $pro_config['money_unit'], $_agency_chossen['percent_sale'], $data_row['num'], true );//gia theo tong sp
                $price = nv_get_price_agency( $price['sale'], $pro_config['money_unit'], $_agency_chossen['percent_sale'], $data_row['num'], false );
            }else{
                $price_all = nv_get_price($data_row['id'], $pro_config['money_unit'], $data_row['num']);
            }
            $xtpl->assign('PRICE', $price);
            $xtpl->assign('PRICE_TOTAL', $price_all);
            ...
            $price_total = $price_total + $price_all['sale'];
            ++$j;
        }
    }
    ...
    if( !empty( $_agency_chossen )){
        $xtpl->assign('LINK_CART', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=book-order/' . $_agency_chossen['alias']);
        if( $data_order['who_is'] > 0 ){
            $xtpl->parse('main.agency_chossen.data_users');
        }
        $xtpl->parse('main.agency_chossen');
    }else{
        $xtpl->assign('LINK_CART', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=cart');
    }
    $xtpl->parse('main');
...
function print_pay($data_content, $data_pro)
{
    ...
    $payment = '';
    if ($data_content['transaction_status'] == 6) {
        $payment = $lang_module['history_shiped_product'];
    } elseif ($data_content['transaction_status'] == 5) {
        $payment = $lang_module['history_shiping_product'];
    } else if ($data_content['transaction_status'] == 4) {
...
function search_result_theme($key, $numRecord, $per_pages, $pages, $array_content, $url_link, $catid)
{
    ...
    if (!empty($array_content)) {
        foreach ($array_content as $value) {
            $url = $global_array_shops_cat[$value['catid']]['link'] . '/' . $value['alias'] . "-" . $value['id'];
...
function book_product_agency( $product_list, $data_content, $agency_info, $agency_chossen )
{
    ...
    $xtpl = new XTemplate('book.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
    $xtpl->assign('LANG', $lang_module);
    $xtpl->assign('TEMPLATE', $module_info['template']);
    $xtpl->assign('NV_BASE_SITEURL', NV_BASE_SITEURL);
        ...
        */
                ...
                */
                    ...
                    for ($i = 0; $i < count($array_group_main); $i++) {
                        foreach ($array_sub_group as $sub_group_id) {
                            ...
                            if ($item['parentid'] == $group_main_id) {
                                ...
                                $xtpl->assign('SUB_GROUP', $data);
                                $xtpl->parse('main.data_order_cart.rows.sub_group.loop');
                            }
                        }
                    }
                    $xtpl->parse('main.data_order_cart.rows.sub_group');
                }
                ...
                if (!empty($data_row['group'])) {
                    ...
                    foreach ($data_row['group'] as $groupid) {
                        ...
                        $items['parent_title'] = $global_array_group[$items['parentid']]['title'];
                        $xtpl->assign('group', $items);
                        $xtpl->parse('main.data_order_cart.rows.display_group.group');
                    }
                    $xtpl->parse('main.data_order_cart.rows.display_group');
                }
        ...
        $xtpl->assign('unit_config',$money_config[$pro_config['money_unit']]['symbol'] );
        $xtpl->assign('LINK_DEL_ALL', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=remove');
        $xtpl->assign('LINK_CART', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=book-order/' . $act);
        $xtpl->assign('LINK_PRODUCTS', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '');
        $xtpl->assign('link_order_all', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=order');
        ...
        if (!empty($order_info)) {
            ...
            if ($module_config[$module_name]['use_coupons']) {
                $xtpl->parse('main.data_order_cart.coupons_code');
                $xtpl->parse('main.data_order_cart.coupons_javascript');
            }
        }
 ...
 * @return
 */
function product_booking( $data_content )
{
    global $module_info, $lang_module, $module_config, $module_file, $module_name, $pro_config, $money_config, $global_array_group, $global_array_shops_cat;
