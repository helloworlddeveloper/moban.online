...
$table_name = NV_PREFIXLANG . '_sm_orders';
...
if (empty($data_content)) {
    Header('Location: ' . NV_BASE_SITEURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=shops');
}
...
$listid = $listnum = $listnumgif = $listprice = array();
$result = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_sm_orders_id WHERE order_id='.$order_id );
...
foreach ($listid as $id) {
    ...
    $sql = 'SELECT t1.*, t2.title AS unit_title FROM ' . NV_PREFIXLANG . '_sm_units AS t2, ' . NV_PREFIXLANG . '_sm_product AS t1 WHERE t1.unit=t2.id AND t1.id =' . $id . ' AND t1.status =1';
    $result = $db->query($sql);
    if ($result->rowCount()) {
        $row = $result->fetch();
...
$result = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_sm_transaction WHERE order_id=' . $order_id . ' ORDER BY transaction_id DESC');
