...
$_agency_chossen = $nv_Request->get_string($module_data . '_agency_chossen', 'session' );
...
if (! empty($_SESSION[$module_data . '_cart'])) {
    foreach ($_SESSION[$module_data . '_cart'] as $pro_id => $info) {
        ...
        $price_agency = nv_get_price_agency( $price['sale'], $pro_config['money_unit'], $_agency_chossen['percent_sale'], $info['num'], true );
        $total = $total + $price_agency['sale'];
    }
    $total_old = $total;
}
...
if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file . '/block.cart.tpl')) {
    $block_css = $global_config['module_theme'];
} else {
    $block_css = 'default';
}

$xtpl = new XTemplate("block.cart.tpl", NV_ROOTDIR . "/themes/" . $block_css . "/modules/" . $module_file);
