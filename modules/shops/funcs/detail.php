...
if ($global_array_shops_cat[$data_content['catid']]['form'] != '') {
    $idtemplate = $db->query('SELECT id FROM ' . $db_config['prefix'] . '_' . $module_data . '_template where alias = "' . preg_replace("/[\_]/", "-", $global_array_shops_cat[$data_content['catid']]['form']) . '"')->fetchColumn();
    if ($idtemplate) {
        $listfield = array();
        $array_tmp = array();
        ...
        if (!empty($listfield)) {
            ...
            $data_content['template'] = $global_array_shops_cat[$data_content['catid']]['form'];
            while ($row = $result->fetch()) {
...
if (nv_user_in_groups($global_array_shops_cat[$catid]['groups_view'])) {
    ...
    // Fetch Limit
    $db->sqlreset()
        ->select(' id, catid, listcatid, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, homeimgfile, homeimgthumb, addtime, publtime, product_code, product_number, product_price, price_config, money_unit, discount_id, showprice, ' . NV_LANG_DATA . '_hometext,' . NV_LANG_DATA . '_gift_content, gift_from, gift_to')
        ->from($db_config['prefix'] . '_' . $module_data . '_rows')
        ->where('id!=' . $id . ' AND catid = ' . $data_content['catid'] . ' AND status=1')
        ->order('ID DESC')
        ->limit($pro_config['per_row'] * 2);
    $result = $db->query($db->sql());
    
    $data_others = array();
    while (list ($_id, $catid, $listcatid, $title, $alias, $homeimgfile, $homeimgthumb, $addtime, $publtime, $product_code, $product_number, $product_price, $price_config, $money_unit, $discount_id, $showprice, $hometext, $gift_content, $gift_from, $gift_to) = $result->fetch(3)) {
        if ($homeimgthumb == 1) {
            // image thumb
        ...
        $data_others[] = array(
            ...
            'showprice' => $showprice,
            'newday' => $global_array_shops_cat[$data_content['catid']]['newday'],
            'gift_content' => $gift_content,
            'gift_from' => $gift_from,
            'gift_to' => $gift_to,
            'link_pro' => NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $global_array_shops_cat[$data_content['catid']]['alias'] . '/' . $alias . $global_config['rewrite_exturl'],
    ...
    if (!empty($_SESSION[$module_data . '_proview'])) {
        ...
        if (!empty($arrtempid)) {
            // Fetch Limit
            $db->sqlreset()
                ->select('id, catid, listcatid, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, homeimgfile, homeimgthumb, addtime, publtime, product_code, product_number, product_price, money_unit, discount_id, showprice, ' . NV_LANG_DATA . '_hometext,' . NV_LANG_DATA . '_gift_content, gift_from, gift_to')
                ->from($db_config['prefix'] . '_' . $module_data . '_rows')
                ->where('id IN ( ' . $arrtempid . ') AND status=1')
                ->order('id DESC')
                ->limit($pro_config['per_row'] * 2);
            $result = $db->query($db->sql());
            while (list ($_id, $catid_i, $listcatid, $title, $alias, $homeimgfile, $homeimgthumb, $addtime, $publtime, $product_code, $product_number, $product_price, $money_unit, $discount_id, $showprice, $hometext, $gift_content, $gift_from, $gift_to) = $result->fetch(3)) {
                if ($homeimgthumb == 1) {
                    // image thumb
                    $thumb = NV_BASE_SITEURL . NV_FILES_DIR . '/' . $module_upload . '/' . $homeimgfile;
                } elseif ($homeimgthumb == 2) {
    ...
    $product_number = $data_content['product_number'] - $data_content['num_sell'];
    $lang_module['product_ok'] = sprintf( $lang_module['product_ok'], $product_number);
