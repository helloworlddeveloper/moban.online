...
if (strlen($key) >= NV_MIN_SEARCH_LENGTH) {
    ...
    $db->select('id, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, catid,  listcatid, ' . NV_LANG_DATA . '_hometext, publtime, homeimgfile, homeimgthumb')->order($orderby)->limit($per_pages)->offset(($page - 1) * $per_page);
    ...
    while (list($id, $title, $alias, $catid_i, $listcatid, $hometext, $publtime, $homeimgfile, $homeimgthumb) = $result->fetch(3)) {
        if ($homeimgthumb == 1) {
            //image thumb
