...
if (! empty($catid)) {
    ...
    $db->sqlreset()->select('id, catid, listcatid, publtime, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, ' . NV_LANG_DATA . '_hometext, homeimgfile')->from($db_config['prefix'] . '_' . $module_data . '_rows')->where('listcatid= ' . $catid . ' AND status =1')->order('publtime DESC')->limit(30);
    $sql = $db->sql();
} else {
    $db->sqlreset()->select('id, catid, listcatid, publtime, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, ' . NV_LANG_DATA . '_hometext, homeimgfile, homeimgthumb')->from($db_config['prefix'] . '_' . $module_data . '_rows')->where('status =1')->order('publtime DESC')->limit(30);
    $sql = $db->sql();
}

if ($module_info['rss']) {
    $result = $db->query($sql);
    while (list($id, $catid_i, $listcatid, $publtime, $title, $alias, $hometext, $homeimgfile, $homeimgthumb) = $result->fetch(3)) {
        $catalias = $global_array_shops_cat[$catid_i]['alias'];
