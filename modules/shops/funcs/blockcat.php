...
if (isset($array_op[1])) {
    ...
    $stmt = $db->prepare('SELECT bid, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, image, ' . NV_LANG_DATA . '_description, ' . NV_LANG_DATA . '_keywords FROM ' . $db_config['prefix'] . '_' . $module_data . '_block_cat WHERE ' . NV_LANG_DATA . '_alias= ' . $db->quote( $alias ) . ' ORDER BY weight DESC');
    ...
    if ($bid > 0) {
        ...
        while ($item = $result->fetch()) {
            ...
            $item['link'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $global_array_shops_cat[$item['catid']]['alias'] . '/' . $item[NV_LANG_DATA . '_alias'] . $global_config['rewrite_exturl'];
