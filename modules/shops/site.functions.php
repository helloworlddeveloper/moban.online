...
function nv_get_price($pro_id, $currency_convert, $number = 1, $per_pro = false, $module = '')
{
    ...
    $product = $db->query('SELECT catid, listcatid, product_price, product_price_sale, money_unit, price_config, discount_id FROM ' . $db_config['prefix'] . '_' . $module_data . '_rows WHERE id = ' . $pro_id)->fetch();
    ...
    if ($global_array_shops_cat[$product['catid']]['typeprice'] == 2) {
    ...
    } elseif ($global_array_shops_cat[$product['catid']]['typeprice'] == 1) {
    ...
    if( $product['product_price_sale'] > 0 ){
        ...
        $discount_percent = round((( $return['price'] - $product['product_price_sale']) / $return['price'] ) * 100 ); 
        $return['discount'] = $product['product_price_sale'];// Số tiền giảm giá sản phẩm chưa format
        $return['discount_format'] = nv_number_format($product['product_price_sale'], $decimals); // Số tiền giảm giá sản phẩm đã format
        $return['discount_percent'] = $discount_percent . '%';
        $return['discount_unit'] = $discount_unit;// Đơn vị giảm giá
        if (!$per_pro) {
            $return['sale'] = $product['product_price_sale'] * $number;// Giá bán thực tế của sản phẩm
        }
        else{
            $return['sale'] = $product['product_price_sale'];// Giá bán thực tế của sản phẩm
        }
    ...
    $return['unit'] = $money_config[$currency_convert]['code'];
    if( isset( $_config['end_time'] )){
        $return['end_time'] = $_config['end_time'];
        $return['time_to_gift'] = date('Y/m/d H:i:s', $_config['end_time']);    
    }else{
        $return['end_time'] = 0;
        $return['time_to_gift'] = 0;
    }
 ...
 * @return
 */
function nv_get_price_agency($price, $currency_convert, $percent_sale = 0, $number = 1, $per_pro = false )
{
    ...
    $decimals = nv_get_decimals($currency_convert);
    $return['sale'] = $price_agency;// Giá nhap cho agency
    $return['sale_format'] = nv_number_format($price_agency, $decimals);
